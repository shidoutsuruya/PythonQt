...
from ui_grahicsVideoItem import Ui_GraphicsVideoItem

class Q(QMainWindow):
    def __init__(self,parent=None):
        super().__init__(parent)
        self.ui=Ui_GraphicsVideoItem()
        self.ui.setupUi(self)
        
        self.player=QMediaPlayer(self)
        self.player.setNotifyInterval(1000)
        self.ui.pbtText.setCheckable(True)
        self.ui.pbtText.setChecked(True)
        scene=QGraphicsScene(self)
        self.ui.graphicsView.setScene(scene)
        #video item setting
        self.videoItem=QGraphicsVideoItem()
        self.videoItem.setSize(QSizeF(600,600))
        ...
        #text setting
        self.textItem=QGraphicsTextItem('hello world')
        font=self.textItem.font()
        font.setPointSize(20)
        self.textItem.setFont(font)
        self.textItem.setDefaultTextColor(Qt.green)
    ...
    @pyqtSlot()
    def on_pbtZoomIn_clicked(self):
        sc=self.videoItem.scale()
        self.videoItem.setScale(sc+10)
    @pyqtSlot()
    def on_pbtZoomOut_clicked(self):
        sc=self.videoItem.scale()
        self.videoItem.setScale(sc-10)
    @pyqtSlot(bool) 
    def on_pbtText_clicked(self,checked):
